...
/* Bắt đầu một thread mới chạy chương trình người dùng được tải từ file_name. Thread mới có thể được lên lịch (và thậm chí có thể thoát)trước khi process_execute() trả về. Trả về tiến trình mới 
   , id luồng hoặc TID_ERROR nếu không thể tạo luồng.*/
tid_t
...
/* Mục đích là mỗi thread sẽ tải một tiến trình của người dùng và khởi động nó */
static void
...
/* Lệnh gọi hệ thống wait(pid) tìm pid của tiến trình con của tiến trình hiện tại và đợi cho đến khi nó kết thúc. */
int
